...
class Actor:
    ...
    #
    def act(self):
        print(f"Iteration {self.Iteration()}: Actor {self.__actorID}")
        self.__itCounter += 1
    ...
    def setLocation(self, x, y):
        ...
        if x < 0 or x >= self.__world.getWidth():
            raise ValueError
        if y < 0 or y >= self.__world.getHeight():
            raise ValueError
    ...
    def __str__(self):
        try:
            st = f"ID = {self.getID()} "u'\u2192 '.encode('utf-8')
            st += f'position = ({self.getX()}, {self.getY()})\n'
        except TypeError:
            st = f"ID = {self.getID()} "u'\u2192 '
            st += f'position = ({self.getX()}, {self.getY()})\n'
        return st
